...
import { expectRecord, expectString, ownPropertyNames, readOwnField, rejectUnknownKeys, } from "../core/validation.mjs";
export const COMPLETION_CANCELLED_CODE = "CANCELLED";
export const COMPLETION_TIMEOUT_CODE = "TIMEOUT";
...
function defineDataProperty(record, name, value, enumerable = value !== undefined) {
...
function normalizeKnownFields(fields) {
    const normalized = {};
    for (const [name, value] of fields) {
        if (value !== undefined)
            defineDataProperty(normalized, name, value);
    }
    return ownDataOnly(normalized);
}
function expectPlainDataRecord(value, label) {
...
function validateTimeoutOptions(value) {
    if (value === undefined)
        return { signal: undefined, timeoutMs: DEFAULT_COMPLETION_TIMEOUT_MS };
    ...
    if (typeof timeoutMs !== "number" ||
        ...
        throw new RangeError(`Completion timeout option 'timeoutMs' must be an integer between 0 and ${MAX_COMPLETION_TIMEOUT_MS}.`);
    }
...
function validateModelSelection(value, label) {
    ...
    if (value.length > MAX_COMPLETION_MODEL_CHARACTERS) {
        throw new CoreValidationError("INPUT_TOO_LARGE", `${label} is ${value.length} characters; limit is ${MAX_COMPLETION_MODEL_CHARACTERS}.`);
    }
    return value;
}
function validateMetadata(value) {
    ...
    for (const key of ownPropertyNames(metadata, "INVALID_EVALUATION", "Completion adapter result metadata")) {
        ...
        if (typeof entry !== "string" &&
            ...
            throw new CoreValidationError("INVALID_EVALUATION", `Completion adapter result metadata field '${key}' must be a string, finite number, or boolean.`);
        }
...
function validateUsage(value) {
    ...
    rejectUnknownKeys(usage, ["inputTokens", "outputTokens"], "INVALID_EVALUATION", "Completion adapter result usage");
    for (const field of ["inputTokens", "outputTokens"]) {
        ...
        if (tokenCount !== undefined &&
            ...
            throw new CoreValidationError("INVALID_EVALUATION", `Completion adapter result usage.${field} must be a non-negative safe integer when present.`);
        }
    }
...
function validateCompletionRequest(request) {
    ...
    rejectUnknownKeys(record, ["fixtureId", "model", "prompt", "maxOutputCharacters"], "INVALID_EVALUATION", "Completion request");
    ...
    if (prompt.length > MAX_COMPLETION_PROMPT_CHARACTERS) {
        throw new CoreValidationError("INPUT_TOO_LARGE", `Completion prompt is ${prompt.length} characters; limit is ${MAX_COMPLETION_PROMPT_CHARACTERS}.`);
    }
    ...
    if (requestedOutputLimit !== undefined &&
        ...
        throw new CoreValidationError("INVALID_EVALUATION", "Completion request maxOutputCharacters must be a positive safe integer.");
    }
    if (requestedOutputLimit !== undefined && requestedOutputLimit > MAX_COMPLETION_OUTPUT_CHARACTERS) {
        throw new CoreValidationError("OUTPUT_TOO_LARGE", `Completion request maxOutputCharacters is ${requestedOutputLimit}; limit is ${MAX_COMPLETION_OUTPUT_CHARACTERS}.`);
    }
    if (requestedOutputLimit !== undefined)
        outputLimit = requestedOutputLimit;
    return {
...
export function normalizeCompletionRequest(request) {
    return validateCompletionRequest(request).request;
}
export function validateCompletionResult(result, outputLimit = MAX_COMPLETION_OUTPUT_CHARACTERS) {
    if (typeof outputLimit !== "number" ||
        !Number.isSafeInteger(outputLimit) ||
        outputLimit <= 0) {
        throw new CoreValidationError("INVALID_EVALUATION", "Completion result outputLimit must be a positive safe integer.");
    }
    if (outputLimit > MAX_COMPLETION_OUTPUT_CHARACTERS) {
        throw new CoreValidationError("OUTPUT_TOO_LARGE", `Completion result outputLimit is ${outputLimit}; limit is ${MAX_COMPLETION_OUTPUT_CHARACTERS}.`);
    }
    const record = expectRecord(result, "INVALID_EVALUATION", "Completion adapter result");
    rejectUnknownKeys(record, ["text", "model", "finishReason", "latencyMs", "metadata", "usage", "costUsd"], "INVALID_EVALUATION", "Completion adapter result");
    ...
    if (text.length > outputLimit) {
        throw new CoreValidationError("OUTPUT_TOO_LARGE", `Completion output is ${text.length} characters; limit is ${outputLimit}.`);
    }
    ...
    if (finishReason !== undefined && typeof finishReason !== "string") {
        throw new CoreValidationError("INVALID_EVALUATION", "Completion adapter result finishReason must be a string when present.");
    }
    const latencyMs = readOwnField(record, "latencyMs");
    if (latencyMs !== undefined &&
        (typeof latencyMs !== "number" || !Number.isSafeInteger(latencyMs) || latencyMs < 0)) {
        throw new CoreValidationError("INVALID_EVALUATION", "Completion adapter result latencyMs must be a non-negative safe integer when present.");
    }
    ...
    if (costUsd !== undefined && (typeof costUsd !== "number" || !Number.isFinite(costUsd) || costUsd < 0)) {
        throw new CoreValidationError("INVALID_EVALUATION", "Completion adapter result costUsd must be a finite non-negative number when present.");
    }
    return normalizeKnownFields([
        ["text", text],
        ["model", model],
...
function validateAdapter(adapter) {
    if (adapter === null || typeof adapter !== "object" || typeof adapter.complete !== "function") {
...
export function throwIfCompletionAborted(signal) {
    if (signal?.aborted)
 ...
 */
export async function completeWithTimeout(adapter, request, options) {
