...
export function matchesFormat(text, format) {
    if (format === "json")
        return isJson(text);
    ...
    if (format === "bullet_list")
        return lines.every((line) => /^[-*+]\s+\S/u.test(line));
    return lines.every((line, index) => {
        const match = /^(\d+)\.\s+\S/u.exec(line);
        return match !== null && Number(match[1]) === index + 1;
    });
}
export function validateDeterministicCriteria(value) {
    const criteria = expectRecord(value, "INVALID_FIXTURE", "Fixture criteria");
    rejectUnknownKeys(criteria, ["expectedFormat", "maxWords", "minWords", "requiredKeywords", "forbiddenPhrases"], "INVALID_FIXTURE", "Fixture criteria");
    const expectedFormatValue = readOwnField(criteria, "expectedFormat");
    if (expectedFormatValue !== undefined &&
        expectedFormatValue !== "json" &&
        expectedFormatValue !== "bullet_list" &&
        expectedFormatValue !== "numbered_list") {
        throw new CoreValidationError("INVALID_FIXTURE", "Unsupported expected format; expected one of json, bullet_list, or numbered_list.");
    }
    ...
    for (const [name, value] of [
        ...
        if (value !== undefined && (typeof value !== "number" || !Number.isSafeInteger(value) || value < 0)) {
            throw new CoreValidationError("INVALID_FIXTURE", `Criterion '${name}' must be a non-negative safe integer.`);
        }
    }
    if (minWordsValue !== undefined &&
        maxWordsValue !== undefined &&
        minWordsValue > maxWordsValue) {
        throw new CoreValidationError("INVALID_FIXTURE", "Criterion 'minWords' cannot exceed 'maxWords'.");
    }
    ...
    for (const [name, terms] of [
        ...
        if (terms !== undefined) {
            ...
            if (totalTermCount > MAX_CRITERION_TERM_COUNT) {
                throw new CoreValidationError("INPUT_TOO_LARGE", `Criterion term count is ${totalTermCount}; limit is ${MAX_CRITERION_TERM_COUNT}.`);
            }
            for (const [index, term] of checkedTerms.entries()) {
                if (typeof term !== "string" || term.trim() === "") {
                    throw new CoreValidationError("INVALID_FIXTURE", `Criterion '${name}' entry at index ${index} must be a non-empty string.`);
                }
                if (term.length > MAX_CRITERION_TERM_CHARACTERS) {
                    throw new CoreValidationError("INPUT_TOO_LARGE", `Criterion '${name}' entry at index ${index} is ${term.length} characters; limit is ${MAX_CRITERION_TERM_CHARACTERS}.`);
                }
                totalTermCharacters += term.length;
                if (totalTermCharacters > MAX_CRITERION_TOTAL_TERM_CHARACTERS) {
                    throw new CoreValidationError("INPUT_TOO_LARGE", `Criterion terms total ${totalTermCharacters} characters; limit is ${MAX_CRITERION_TOTAL_TERM_CHARACTERS}.`);
                }
            }
            const normalizedTerms = Object.freeze(checkedTerms.map((term) => term));
            if (name === "requiredKeywords")
                requiredKeywords = normalizedTerms;
            else
                forbiddenPhrases = normalizedTerms;
        }
    }
    const expectedFormat = expectedFormatValue;
    const maxWords = maxWordsValue;
    const minWords = minWordsValue;
    return {
...
export function checkFixtureOutput(text, criteria) {
    expectString(text, "INVALID_FIXTURE", "Fixture output text");
    if (text.length > MAX_FIXTURE_CHECK_CHARACTERS) {
        throw new CoreValidationError("INPUT_TOO_LARGE", `Fixture output is ${text.length} characters; limit is ${MAX_FIXTURE_CHECK_CHARACTERS}.`);
    }
    const checkedCriteria = validateDeterministicCriteria(criteria);
    const wordCount = countWords(text);
    const checks = [];
    const expectedFormat = readOwnField(checkedCriteria, "expectedFormat");
    const maxWords = readOwnField(checkedCriteria, "maxWords");
    const minWords = readOwnField(checkedCriteria, "minWords");
    const requiredKeywords = readOwnField(checkedCriteria, "requiredKeywords");
    const forbiddenPhrases = readOwnField(checkedCriteria, "forbiddenPhrases");
