...
import { expectDenseArray, expectRecord, readOwnField, rejectUnknownKeys } from "./validation.mjs";
const TIE_BREAKERS = Object.freeze(["totalTokens", "latencyMs", "costUsd"]);
/**
 ...
 */
export const DEFAULT_RANKING_OBJECTIVE = Object.freeze({
    deterministicWeight: 1,
    judgeWeight: 0,
    tieBreakers: TIE_BREAKERS,
});
export function validateRankingObjective(value) {
    if (value === undefined)
        return DEFAULT_RANKING_OBJECTIVE;
    ...
    const weights = { deterministicWeight: 1, judgeWeight: 0 };
    for (const name of ["deterministicWeight", "judgeWeight"]) {
    ...
    if (tieBreakersValue !== undefined) {
        ...
        const seen = new Set();
        tieBreakers = Object.freeze(entries.map((entry, index) => {
            if (!TIE_BREAKERS.includes(entry)) {
                throw new CoreValidationError("INVALID_OBJECTIVE", `Ranking objective tieBreakers[${index}] must be one of ${TIE_BREAKERS.join(", ")}.`);
            }
            if (seen.has(entry)) {
 ...
 * imputed as zero.
 */
export function combineQuality(deterministic, judge, objective) {
    const parts = [];
    if (objective.deterministicWeight > 0)
        parts.push({ weight: objective.deterministicWeight, value: deterministic, label: "deterministic" });
    if (objective.judgeWeight > 0)
        parts.push({ weight: objective.judgeWeight, value: judge, label: "judge" });
    const unknown = parts.filter((part) => part.value.status === "unknown");
    if (unknown.length > 0) {
        return Object.freeze({
            status: "unknown",
            reason: unknown.map((part) => `${part.label} score is unknown (${part.value.reason})`).join("; "),
        });
    }
    let weighted = 0;
    let total = 0;
 ...
 */
function compareMeasuredAsc(left, right) {
    if (left.status === "measured" && right.status === "measured")
...
function qualityEqual(left, right) {
    return compareMeasuredDesc(left.combined, right.combined) === 0;
}
function tieBreakerUsed(left, right, objective) {
    for (const breaker of objective.tieBreakers) {
        if (compareMeasuredAsc(left[breaker], right[breaker]) !== 0)
            return breaker;
    }
    return null;
}
function compareRows(left, right, objective) {
 ...
 */
export function rankCandidates(evaluations, objective) {
    ...
    const rows = entries.map((entry, index) => {
        ...
        const measurement = (name) => {
            ...
            if (value !== null && typeof value === "object") {
                ...
                if (status === "measured" && typeof number === "number" && Number.isFinite(number))
                    return Object.freeze({ status: "measured", value: number });
                const reason = readOwnField(value, "reason");
                if (status === "unknown" && typeof reason === "string")
    ...
    for (const row of rows) {
        if (ids.has(row.candidateId))
            throw new CoreValidationError("INVALID_EVALUATION", `Candidate id '${row.candidateId}' is duplicated.`);
    ...
    for (const [index, row] of sorted.entries()) {
        ...
        if (previous !== undefined && !qualityEqual(previous, row))
            currentRank = index + 1;
