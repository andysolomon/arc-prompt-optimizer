...
import { MAX_ARRAY_ENTRIES, ownPropertyNames } from "./validation.mjs";
...
function unsupported(path, reason) {
    throw new CoreValidationError("NON_CANONICAL_VALUE", `Cannot canonically serialize value at ${path}: ${reason}.`);
}
function depthOverflow(path, depth) {
    return unsupported(path, `nesting depth ${depth} exceeds limit of ${MAX_CANONICAL_DEPTH}`);
}
function sizeOverflow(path, size) {
    return unsupported(path, `serialized output would be ${size} characters; limit is ${MAX_CANONICAL_SERIALIZED_CHARACTERS}`);
}
function append(state, text, path) {
    const nextSize = state.characters + text.length;
    if (nextSize > MAX_CANONICAL_SERIALIZED_CHARACTERS)
        sizeOverflow(path, nextSize);
    state.output.push(text);
    state.characters = nextSize;
}
function appendJsonString(state, value, path) {
    append(state, '"', path);
    for (const character of value) {
        const encoded = JSON.stringify(character);
        if (typeof encoded !== "string")
            unsupported(path, "string encoding failed");
...
function ownDataPropertyNames(value, path, isArray = false) {
    const names = [];
    for (const key of ownPropertyNames(value, "NON_CANONICAL_VALUE", `Canonical value at ${path}`, isArray)) {
...
function serialize(value, path, depth, ancestors, state) {
    if (depth > MAX_CANONICAL_DEPTH)
        depthOverflow(path, depth);
    if (value === null) {
    ...
    if (typeof value === "number") {
        if (!Number.isFinite(value))
            unsupported(path, "numbers must be finite");
        append(state, Object.is(value, -0) ? "0" : JSON.stringify(value), path);
        return;
    }
    if (typeof value !== "object")
        unsupported(path, `unsupported type '${typeof value}'`);
    if (ancestors.has(value))
        unsupported(path, "circular reference");
    ancestors.add(value);
    try {
...
export function canonicalStringify(value) {
    const state = { output: [], characters: 0 };
