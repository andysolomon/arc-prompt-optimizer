...
export function ownPropertyNames(value, code, label, excludeArrayLength = false) {
    ...
    if (names.length > MAX_OWN_PROPERTY_COUNT) {
        throw new CoreValidationError(code, `${label} has ${names.length} own properties; limit is ${MAX_OWN_PROPERTY_COUNT}.`);
    }
    let nameCharacters = 0;
    for (const name of names) {
        nameCharacters += name.length;
        if (nameCharacters > MAX_OWN_PROPERTY_NAME_CHARACTERS) {
            throw new CoreValidationError(code, `${label} has ${nameCharacters} property-name characters; limit is ${MAX_OWN_PROPERTY_NAME_CHARACTERS}.`);
        }
    }
    return names.sort();
}
export function hasOwnField(record, name) {
    return Object.getOwnPropertyDescriptor(record, name) !== undefined;
}
export function readOwnField(record, name) {
    const descriptor = Object.getOwnPropertyDescriptor(record, name);
    return descriptor && "value" in descriptor ? descriptor.value : undefined;
}
export function expectRecord(value, code, label) {
...
export function expectDenseArray(value, code, label) {
    ...
    if (!Number.isSafeInteger(length) || length < 0 || length > MAX_ARRAY_ENTRIES) {
        throw new CoreValidationError(code, `${label} has ${String(length)} entries; limit is ${MAX_ARRAY_ENTRIES}.`);
    }
    const indices = [];
    const entries = [];
...
export function expectString(value, code, label) {
    if (typeof value !== "string") {
        throw new CoreValidationError(code, `${label} must be a string.`);
    }
    return value;
}
export function rejectUnknownKeys(record, allowed, code, label) {
