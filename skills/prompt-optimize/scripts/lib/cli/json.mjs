...
function appendJsonString(state, value, path) {
    ...
    if (typeof encoded !== "string")
        jsonError(path, "string encoding failed");
...
function serialize(value, path, depth, ancestors, state) {
    if (depth > MAX_CLI_JSON_DEPTH)
        jsonError(path, `nesting depth ${depth} exceeds limit of ${MAX_CLI_JSON_DEPTH}`);
    if (value === null) {
    ...
    if (typeof value === "number") {
        if (!Number.isFinite(value))
            jsonError(path, "numbers must be finite");
        append(state, Object.is(value, -0) ? "0" : JSON.stringify(value), path);
        return;
    }
    if (typeof value !== "object")
        jsonError(path, `unsupported type '${typeof value}'`);
    if (ancestors.has(value))
        jsonError(path, "circular references are not supported");
    ancestors.add(value);
    try {
        if (Array.isArray(value)) {
            ...
            for (let index = 0; index < value.length; index += 1) {
                ...
                if (!Object.hasOwn(value, index))
                    jsonError(`${path}[${index}]`, "sparse arrays are not supported");
        ...
        if (Object.getOwnPropertySymbols(value).length > 0)
            jsonError(path, "symbol-keyed properties are not supported");
...
export function stringifyCliJson(value) {
    const state = { output: [], characters: 0 };
    serialize(value, "$", 0, new Set(), state);
    return state.output.join("");
}
export function stringifyCliJsonLine(value) {
    const json = stringifyCliJson(value);
    const sizeWithNewline = json.length + 1;
    if (sizeWithNewline > MAX_CLI_JSON_CHARACTERS)
        sizeOverflow("$", sizeWithNewline);
    return `${json}\n`;
}
