...
function table(headers, rows) {
    ...
    const format = (row) => row.map((cell, index) => cell.padEnd(widths[index] ?? 0)).join("  ");
    return [format(headers), format(widths.map((width) => "-".repeat(width))), ...rows.map(format)].join("\n");
}
function rankRows(ranking) {
...
function candidateRows(evaluations) {
...
export function redactSuiteEvaluation(result) {
    return {
...
export function redactOptimization(result) {
    return {
...
export function fullOptimizationForOutput(result) {
    return {
        budget: result.budget,
        candidates: result.candidates,
        completionsUsed: result.completionsUsed,
...
export function fullEvaluationForOutput(candidate, result) {
    return {
...
export function formatPatternsJson(patterns) {
    return formatJson({
...
export function formatPatternsText(patterns) {
    return `${table(["Name", "Display", "Variables", "Temp"], patterns.map((pattern) => [
        pattern.name,
        pattern.displayName,
...
export function formatModelsText(models) {
    return `${table(["Model", "Provider", "Id"], models.map((model) => [model.canonical, model.provider, model.id]))}\n`;
}
export function formatEvaluationText(result, outputPath) {
    const sections = [
...
export function formatOptimizationText(result, outputPath) {
    const sections = [
        `Completions: ${result.completionsUsed}`,
        "",
        "Ranking",
...
export function generatedSpecsSummary(generated) {
    return {
...
export function formatCandidatesJson(candidates) {
    return formatJson({
        candidates: candidates.map((candidate) => ({ id: candidate.id, pattern: candidate.pattern, prompt: candidate.prompt })),
    });
}
export function formatCandidatesText(candidates) {
    const sections = candidates.map((candidate) => [`== ${candidate.id} (${candidate.label}, pattern: ${candidate.pattern})`, candidate.prompt].join("\n"));
    return `${sections.join("\n\n")}\n`;
}
...
export function formatScoreText(result) {
    const sections = [
        `Suite: ${result.suiteId}`,
        "",
        "Ranking",
