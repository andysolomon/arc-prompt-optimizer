...
import { buildPreviewCandidates } from "../core/preview.js";
import { DEFAULT_RANKING_OBJECTIVE, rankCandidates } from "../core/ranking.js";
import type {
  CandidateEvaluation,
  CaseEvaluation,
  EvaluationSuite,
...
import { expectDenseArray, expectRecord, expectString, hasOwnField, readOwnField, rejectUnknownKeys } from "../core/validation.js";
...
const MEASUREMENT_KEYS = ["latencyMs", "inputTokens", "outputTokens", "costUsd"] as const;

export interface HarnessCandidateView {
  readonly id: string;
...
export interface HarnessCandidateOutputs {
  readonly id: string;
  readonly outputs: Readonly<Record<string, string>>;
  readonly measurements: Readonly<Record<string, Readonly<Partial<Record<(typeof MEASUREMENT_KEYS)[number], number>>>>>;
}

export interface HarnessScoreResult {
...
function candidateLabel(candidate: PromptCandidate): string {
  const label = candidate.metadata?.["label"];
  return typeof label === "string" ? label : candidate.prompt.pattern;
}

export function harnessCandidates(prompt: string): readonly HarnessCandidateView[] {
  return Object.freeze(
    buildPreviewCandidates(prompt).map((candidate) =>
      Object.freeze({
        id: candidate.id,
...
function validateMeasurement(value: unknown, label: string): Partial<Record<(typeof MEASUREMENT_KEYS)[number], number>> {
  ...
  const checked: Partial<Record<(typeof MEASUREMENT_KEYS)[number], number>> = {};
  for (const key of MEASUREMENT_KEYS) {
    ...
    if (typeof field !== "number" || !Number.isFinite(field) || field < 0 || (integer && !Number.isSafeInteger(field))) {
      throw new CoreValidationError(
        "INVALID_EVALUATION",
        `${label} '${key}' must be a non-negative ${integer ? "safe integer" : "finite number"}.`,
      );
    }
    checked[key] = field;
  }
...
export function validateHarnessOutputs(value: unknown, suite: EvaluationSuite): readonly HarnessCandidateOutputs[] {
  ...
  if (entries.length === 0) throw new CoreValidationError("INVALID_EVALUATION", "Outputs JSON must include at least one candidate.");
  ...
  if (scored > MAX_EVALUATION_COMPLETIONS) {
    throw new CoreValidationError(
      "BUDGET_EXCEEDED",
      `Scoring ${entries.length} candidates against ${suite.cases.length} cases needs ${scored} outputs; limit is ${MAX_EVALUATION_COMPLETIONS}.`,
    );
  }
  ...
  return Object.freeze(
    entries.map((entry, index) => {
      ...
      if (ids.has(id)) throw new CoreValidationError("INVALID_EVALUATION", `Candidate id '${id}' is duplicated.`);
      ids.add(id);
      ...
      for (const caseId of caseIds) {
        ...
        if (text.length > MAX_COMPLETION_OUTPUT_CHARACTERS) {
          throw new CoreValidationError(
            "OUTPUT_TOO_LARGE",
            `Candidate '${id}' output for case '${caseId}' is ${text.length} characters; limit is ${MAX_COMPLETION_OUTPUT_CHARACTERS}.`,
          );
        }
        outputs[caseId] = text;
      }
      const measurements: Record<string, Partial<Record<(typeof MEASUREMENT_KEYS)[number], number>>> = {};
      if (hasOwnField(record, "measurements")) {
        ...
        for (const caseId of caseIds) {
          if (!hasOwnField(measurementRecord, caseId)) continue;
...
function completionLike(measurement: Partial<Record<(typeof MEASUREMENT_KEYS)[number], number>> | undefined): object {
...
export function scoreHarnessOutputs(suite: EvaluationSuite, value: unknown): HarnessScoreResult {
  const candidates = validateHarnessOutputs(value, suite);
  const evaluations: CandidateEvaluation[] = candidates.map((candidate) => {
    const cases: CaseEvaluation[] = suite.cases.map((evaluationCase) => {
      ...
      return Object.freeze({
        ...
        judge: Object.freeze({ status: "not_requested" as const }),
