...
import {
  calculateRenderedLength,
  joinRenderedTemplate,
  parseTemplate,
...
export const EXPLICIT_GENERATION_GUIDANCE: PromptGenerationGuidance = Object.freeze({
  activation: "explicit",
  automatic: false,
  notes:
    "Candidate generation must be explicitly requested by the caller; rendering never intercepts, submits, or rewrites prompts automatically.",
});

function pattern<Name extends PatternName>(value: PromptPattern<Name>): PromptPattern<Name> {
...
export const PROMPT_PATTERNS: readonly PromptPattern<PatternName>[] = Object.freeze([
  pattern({
    name: "persona",
  ...
  pattern({
    ...
    template:
      "Extract information from <source_text> and fill every field in <output_template>. If information is unavailable, write 'N/A'.\n\n<source_text>\n{{text}}\n</source_text>\n\n<output_template>\n{{template_structure}}\n</output_template>",
  ...
  pattern({
    ...
    template:
      "Explain <concept>{{concept}}</concept> for this audience: <audience>{{audience}}</audience>.\n\nConstraints:\n- Use vocabulary appropriate for the audience.\n- Length: {{length}}\n- Include: {{include}}\n- Exclude: {{exclude}}",
...
function validateLimits(value: unknown): RenderLimits {
  ...
  const checked = {} as Record<keyof RenderLimits, number>;
  for (const name of [
    ...
    if (!Number.isSafeInteger(limit) || (limit as number) <= 0) {
      throw new CoreValidationError(
        "INVALID_PATTERN",
        `Render limit '${name}' must be a positive safe integer.`,
      );
    }
...
function validateGenerationGuidance(value: unknown, patternName: string): PromptGenerationGuidance | undefined {
  if (value === undefined) return undefined;
  const guidance = expectRecord(value, "INVALID_PATTERN", `Pattern '${patternName}' generationGuidance`);
  rejectUnknownKeys(
    guidance,
    ["activation", "automatic", "notes"],
    "INVALID_PATTERN",
    `Pattern '${patternName}' generationGuidance`,
  );
  ...
  if (activation !== "explicit" || automatic !== false) {
    throw new CoreValidationError(
      "INVALID_PATTERN",
      `Pattern '${patternName}' generationGuidance must be explicit and non-automatic.`,
    );
  }
  const notes = expectString(
    readOwnField(guidance, "notes"),
    "INVALID_PATTERN",
    `Pattern '${patternName}' generationGuidance.notes`,
  );
  return Object.freeze({ activation: "explicit", automatic: false, notes });
}

function validatePattern(value: unknown): PromptPattern {
  const definition = expectRecord(value, "INVALID_PATTERN", "Pattern definition");
  rejectUnknownKeys(
    definition,
    ["name", "displayName", "description", "template", "variables", "recommendedTemperature", "generationGuidance"],
    "INVALID_PATTERN",
    "Pattern definition",
  );
  const name = expectString(readOwnField(definition, "name"), "INVALID_PATTERN", "Pattern name");
  const displayName = expectString(
    readOwnField(definition, "displayName"),
    "INVALID_PATTERN",
    `Pattern '${name}' displayName`,
  );
  const description = expectString(
    readOwnField(definition, "description"),
    "INVALID_PATTERN",
    `Pattern '${name}' description`,
  );
  const template = expectString(
    readOwnField(definition, "template"),
    "INVALID_PATTERN",
  ...
  if (!/^[a-z][a-z0-9_]*$/.test(name)) {
    throw new CoreValidationError(
      "INVALID_PATTERN",
      `Pattern name '${name}' must match [a-z][a-z0-9_]*.`,
    );
  }
  ...
  if (typeof recommendedTemperature !== "number" || !Number.isFinite(recommendedTemperature)) {
    throw new CoreValidationError(
      "INVALID_PATTERN",
      `Pattern '${name}' recommendedTemperature must be a finite number.`,
    );
  }

  const declared = expectDenseArray(
    readOwnField(definition, "variables"),
    "INVALID_PATTERN",
    `Pattern '${name}' variables`,
  ).map((variable, index) =>
    expectString(variable, "INVALID_PATTERN", `Pattern '${name}' variable at index ${index}`),
  );
  for (const variable of declared) {
    if (!/^[A-Za-z][A-Za-z0-9_]*$/.test(variable)) {
      throw new CoreValidationError(
        "INVALID_PATTERN",
        `Pattern '${name}' variable '${variable}' must match [A-Za-z][A-Za-z0-9_]*.`,
      );
    }
  }
  ...
  if (uniqueDeclared.size !== declared.length) {
    throw new CoreValidationError(
      "INVALID_PATTERN",
      `Pattern '${name}' declares duplicate variables.`,
    );
  }
  ...
  if (missingFromTemplate.length > 0 || undeclared.length > 0) {
    throw new CoreValidationError(
      "INVALID_PATTERN",
      `Pattern '${name}' variable mismatch: unused declarations [${missingFromTemplate.join(", ")}]; undeclared template variables [${undeclared.join(", ")}].`,
    );
  }
  const generationGuidance = validateGenerationGuidance(
...
export class PatternCatalog {
  readonly #patterns: ReadonlyMap<string, PromptPattern>;
  readonly #limits: RenderLimits;

  constructor(
    definitions: readonly PromptPattern[],
          ...
          ...(readOwnField(definition, "generationGuidance") === undefined
            ...
            : {
                generationGuidance: Object.freeze({
                  ...(readOwnField(definition, "generationGuidance") as PromptGenerationGuidance),
                }),
              }),
        }),
      );
  ...
  render(name: string, variables: Readonly<Record<string, string>>): RenderedPrompt {
    ...
    const supplied = ownPropertyNames(
      checkedVariables,
      "INVALID_VARIABLE_VALUE",
      `Variables for pattern '${name}'`,
    );
    const expected = new Set(definition.variables);
    const missing = definition.variables.filter(
      (variable) => !hasOwnField(checkedVariables, variable),
    );
    if (missing.length > 0) {
      throw new CoreValidationError(
        "MISSING_VARIABLES",
        `Missing variables for pattern '${name}': ${[...missing].sort().join(", ")}.`,
      );
    }
    ...
    if (unknown.length > 0) {
      throw new CoreValidationError(
        "UNKNOWN_VARIABLES",
        `Unknown variables for pattern '${name}': ${unknown.join(", ")}. Expected: ${[...definition.variables].sort().join(", ")}.`,
      );
    }

    let totalCharacters = 0;
    for (const variable of definition.variables) {
      const value: unknown = readOwnField(checkedVariables, variable);
      if (typeof value !== "string") {
        throw new CoreValidationError(
          "INVALID_VARIABLE_VALUE",
          `Variable '${variable}' for pattern '${name}' must be a string.`,
        );
      }
      if (value.length > this.#limits.maxVariableCharacters) {
        throw new CoreValidationError(
          "INPUT_TOO_LARGE",
          `Variable '${variable}' for pattern '${name}' is ${value.length} characters; limit is ${this.#limits.maxVariableCharacters}.`,
        );
      }
    ...
    if (totalCharacters > this.#limits.maxTotalVariableCharacters) {
      throw new CoreValidationError(
        "INPUT_TOO_LARGE",
        `Variables for pattern '${name}' total ${totalCharacters} characters; limit is ${this.#limits.maxTotalVariableCharacters}.`,
      );
    }

    const tokens = parseTemplate(definition.template);
    const stringVariables = checkedVariables as Readonly<Record<string, string>>;
    const renderedLength = calculateRenderedLength(tokens, stringVariables);
    if (renderedLength > this.#limits.maxRenderedCharacters) {
      throw new CoreValidationError(
        "OUTPUT_TOO_LARGE",
        `Rendered pattern '${name}' is ${renderedLength} characters; limit is ${this.#limits.maxRenderedCharacters}.`,
      );
    }
...
export function listPatterns(): readonly PromptPattern[] {
  return defaultPatternCatalog.list();
}

export function getPattern(name: string): PromptPattern {
