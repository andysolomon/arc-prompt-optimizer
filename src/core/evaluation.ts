...
import {
  ...
  matchesFormat,
  nonEmptyLines,
  validateDeterministicCriteria,
} from "./fixture-checks.js";
import { buildJudgeRequest, runSemanticJudge } from "./judge.js";
...
import type {
  ...
  SuiteEvaluation,
  WeightedCriterionCheck,
  WeightedCriterionDefinition,
} from "./types.js";
...
const BUILTIN_CRITERION_NAMES: readonly BuiltinCriterionName[] = Object.freeze([
  "format",
  "max_words",
  "min_words",
  "required_keyword",
...
const CUSTOM_KINDS: readonly CustomCriterionKind[] = Object.freeze([
  "includes",
  "excludes",
  "equals",
  "starts_with",
...
function validateWeight(label: string, value: unknown): number {
...
function validateOptionalNonNegativeInteger(label: string, value: unknown): number | undefined {
...
function validateJsonShape(value: unknown): JsonShapeCriterion | undefined {
  if (value === undefined) return undefined;
  ...
  let requiredKeys: readonly string[] | undefined;
  if (keysValue !== undefined) {
    ...
    if (keys.length > MAX_JSON_SHAPE_KEYS) {
      throw new CoreValidationError(
        "INPUT_TOO_LARGE",
        `Criterion 'jsonShape.requiredKeys' has ${keys.length} entries; limit is ${MAX_JSON_SHAPE_KEYS}.`,
      );
    }
  ...
  return Object.freeze({
    ...(rootType === undefined ? {} : { rootType: rootType as "object" | "array" }),
    ...(requiredKeys === undefined ? {} : { requiredKeys }),
  });
}

function validateListShape(value: unknown): ListShapeCriterion | undefined {
  if (value === undefined) return undefined;
...
function validateWeights(value: unknown): Readonly<Partial<Record<BuiltinCriterionName, number>>> | undefined {
  if (value === undefined) return undefined;
  ...
  const checked: Partial<Record<BuiltinCriterionName, number>> = {};
  for (const name of ownPropertyNames(weights, "INVALID_CRITERIA", "Criterion 'weights'")) {
    checked[name as BuiltinCriterionName] = validateWeight(`Criterion weight '${name}'`, readOwnField(weights, name));
  }
  return Object.freeze(checked);
}

function validateCustom(value: unknown): readonly WeightedCriterionDefinition[] | undefined {
  if (value === undefined) return undefined;
  const entries = expectDenseArray(value, "INVALID_CRITERIA", "Criterion 'custom'");
  if (entries.length > MAX_CUSTOM_CRITERIA) {
    throw new CoreValidationError(
      "INPUT_TOO_LARGE",
      `Criterion 'custom' has ${entries.length} entries; limit is ${MAX_CUSTOM_CRITERIA}.`,
    );
  }
  ...
  const checked = entries.map((entry, index) => {
    ...
    if (!CRITERION_ID.test(id) || id.length > 128) invalid(`${label} id '${id}' must match [A-Za-z][A-Za-z0-9_.-]* and be at most 128 characters.`);
    if (ids.has(id)) invalid(`Custom criterion id '${id}' is duplicated.`);
    ids.add(id);
    const weight = validateWeight(`${label} weight`, readOwnField(definition, "weight"));
    const kind = readOwnField(definition, "kind");
    if (!CUSTOM_KINDS.includes(kind as CustomCriterionKind)) {
    ...
    if (term === "") invalid(`${label} value cannot be empty.`);
    if (term.length > MAX_CRITERION_TERM_CHARACTERS) {
      throw new CoreValidationError(
        "INPUT_TOO_LARGE",
        `${label} value is ${term.length} characters; limit is ${MAX_CRITERION_TERM_CHARACTERS}.`,
      );
    }
    ...
    return Object.freeze({
      id,
      weight,
      kind: kind as CustomCriterionKind,
      value: term,
...
const PHASE1_KEYS = ["expectedFormat", "maxWords", "minWords", "requiredKeywords", "forbiddenPhrases"] as const;
const PHASE2_KEYS = ["exactMatch", "maxCharacters", "minCharacters", "jsonShape", "listShape", "weights", "custom"] as const;

export function validateEvaluationCriteria(value: unknown): EvaluationCriteria {
  ...
  for (const key of PHASE1_KEYS) {
    if (hasOwnField(criteria, key)) phase1[key] = readOwnField(criteria, key);
  }
  let base;
  try {
  ...
  if (exactMatch !== undefined) {
    ...
    if ((exactMatch as string).length > MAX_FIXTURE_CHECK_CHARACTERS) {
      throw new CoreValidationError(
        "INPUT_TOO_LARGE",
        `Criterion 'exactMatch' is ${(exactMatch as string).length} characters; limit is ${MAX_FIXTURE_CHECK_CHARACTERS}.`,
      );
    }
  }
  ...
  return Object.freeze({
    ...base,
    ...(exactMatch === undefined ? {} : { exactMatch: exactMatch as string }),
...
function customPassed(text: string, definition: WeightedCriterionDefinition): boolean {
 ...
 */
export function evaluateOutput(text: string, criteria: EvaluationCriteria): DeterministicEvaluation {
  expectString(text, "INVALID_EVALUATION", "Candidate output text");
  if (text.length > MAX_FIXTURE_CHECK_CHARACTERS) {
    throw new CoreValidationError(
      "INPUT_TOO_LARGE",
      `Candidate output is ${text.length} characters; limit is ${MAX_FIXTURE_CHECK_CHARACTERS}.`,
    );
  }
  const checked = validateEvaluationCriteria(criteria);
  const weights = (readOwnField(checked, "weights") ?? {}) as Partial<Record<BuiltinCriterionName, number>>;
  const weightOf = (name: BuiltinCriterionName): number => {
    const override = readOwnField(weights, name);
    return typeof override === "number" ? override : 1;
  };
  const wordCount = countWords(text);
  const characterCount = text.length;
  const checks: WeightedCriterionCheck[] = [];
  const push = (
    id: string,
    criterion: BuiltinCriterionName,
    passed: boolean,
  ...
  const expectedFormat = readOwnField(checked, "expectedFormat") as ExpectedFormat | undefined;
  if (expectedFormat !== undefined) {
    const passed = matchesFormat(text, expectedFormat);
    push("format", "format", passed, expectedFormat, passed);
  }
  const maxWords = readOwnField(checked, "maxWords") as number | undefined;
  if (maxWords !== undefined) push("max_words", "max_words", wordCount <= maxWords, maxWords, wordCount);
  const minWords = readOwnField(checked, "minWords") as number | undefined;
  if (minWords !== undefined) push("min_words", "min_words", wordCount >= minWords, minWords, wordCount);

  const foldedText = text.toLowerCase();
  const requiredKeywords = (readOwnField(checked, "requiredKeywords") ?? []) as readonly string[];
  ...
  const forbiddenPhrases = (readOwnField(checked, "forbiddenPhrases") ?? []) as readonly string[];
  ...
  const exactMatch = readOwnField(checked, "exactMatch") as string | undefined;
  if (exactMatch !== undefined) {
    const passed = text.trim() === exactMatch.trim();
    push("exact_match", "exact_match", passed, exactMatch, passed);
  }
  const maxCharacters = readOwnField(checked, "maxCharacters") as number | undefined;
  if (maxCharacters !== undefined) {
    push("max_characters", "max_characters", characterCount <= maxCharacters, maxCharacters, characterCount);
  }
  const minCharacters = readOwnField(checked, "minCharacters") as number | undefined;
  if (minCharacters !== undefined) {
    push("min_characters", "min_characters", characterCount >= minCharacters, minCharacters, characterCount);
  }

  const jsonShape = readOwnField(checked, "jsonShape") as JsonShapeCriterion | undefined;
  if (jsonShape !== undefined) {
    const parsed = parseJsonSafely(text);
    const rootType = readOwnField(jsonShape, "rootType") as "object" | "array" | undefined;
    const requiredKeys = (readOwnField(jsonShape, "requiredKeys") ?? []) as readonly string[];
    ...
    if (parsed.ok) {
      ...
      if (rootType === "object" && !isObject) passed = false;
      if (rootType === "array" && !isArray) passed = false;
      if (requiredKeys.length > 0) {
        if (!isObject) passed = false;
        else {
          const missing = requiredKeys.filter((key) => !hasOwnField(value as object, key));
          if (missing.length > 0) {
            passed = false;
  ...
  const listShape = readOwnField(checked, "listShape") as ListShapeCriterion | undefined;
  if (listShape !== undefined) {
    const items = listItems(text).length;
    const minItems = readOwnField(listShape, "minItems") as number | undefined;
    const maxItems = readOwnField(listShape, "maxItems") as number | undefined;
    const passed =
      items > 0 && (minItems === undefined || items >= minItems) && (maxItems === undefined || items <= maxItems);
    push("list_shape", "list_shape", passed, `${minItems ?? 1}..${maxItems ?? "*"}`, items);
  }

  const custom = (readOwnField(checked, "custom") ?? []) as readonly WeightedCriterionDefinition[];
...
function validateOptionalText(value: unknown, label: string, limit: number): string | undefined {
  if (value === undefined) return undefined;
...
export function validateEvaluationCase(value: unknown): EvaluationCase {
  const record = expectRecord(value, "INVALID_EVALUATION", "Evaluation case");
  rejectUnknownKeys(
    record,
    ["id", "name", "completionFixtureId", "criteria", "input", "expectedOutput", "judgeRubric"],
    "INVALID_EVALUATION",
    "Evaluation case",
  );
  ...
  const completionFixtureId = expectString(
    readOwnField(record, "completionFixtureId"),
    "INVALID_EVALUATION",
    `Evaluation case '${id}' completionFixtureId`,
  );
  ...
  const expectedOutput = validateOptionalText(
    readOwnField(record, "expectedOutput"),
    `Evaluation case '${id}' expectedOutput`,
    MAX_CASE_INPUT_CHARACTERS,
  );
  const judgeRubric = validateOptionalText(
    readOwnField(record, "judgeRubric"),
    `Evaluation case '${id}' judgeRubric`,
    MAX_CASE_INPUT_CHARACTERS,
  );
  return Object.freeze({
    id,
...
export function validateEvaluationSuite(value: unknown): EvaluationSuite {
  ...
  if (cases.length > MAX_EVALUATION_CASES) {
    throw new CoreValidationError(
      "BUDGET_EXCEEDED",
      `Evaluation suite '${id}' has ${cases.length} cases; limit is ${MAX_EVALUATION_CASES}.`,
    );
  }
  const ids = new Set<string>();
...
export function validatePromptCandidate(value: unknown): PromptCandidate {
  ...
  if (text.length > MAX_EVALUATION_PROMPT_CHARACTERS) {
    throw new CoreValidationError(
      "INPUT_TOO_LARGE",
      `Prompt candidate '${id}' prompt is ${text.length} characters; limit is ${MAX_EVALUATION_PROMPT_CHARACTERS}.`,
    );
  }
  const variablesUsed = expectDenseArray(
    readOwnField(prompt, "variablesUsed"),
    "INVALID_CANDIDATE",
    `Prompt candidate '${id}' prompt.variablesUsed`,
  ).map((entry, index) =>
    expectString(entry, "INVALID_CANDIDATE", `Prompt candidate '${id}' prompt.variablesUsed[${index}]`),
  );
  ...
  return Object.freeze({
    ...
    origin,
    ...(parentCandidateId === undefined ? {} : { parentCandidateId: parentCandidateId as string }),
...
export function unknownMeasurement(reason: string): Measurement {
  return Object.freeze({ status: "unknown", reason });
}

function nonNegativeMeasurement(value: unknown, label: string, integer: boolean): Measurement {
  if (value === undefined) return unknownMeasurement(`${label} was not reported`);
...
export function measureCompletion(result: object): OperationalMeasurements {
  const latencyMs = nonNegativeMeasurement(readOwnField(result, "latencyMs"), "latencyMs", true);
  const usage = readOwnField(result, "usage");
  const usageRecord = usage !== null && typeof usage === "object" && !Array.isArray(usage) ? (usage as object) : undefined;
  ...
  const outputTokens = usageRecord
    ...
    : unknownMeasurement("usage was not reported");
  const totalTokens =
    inputTokens.status === "measured" && outputTokens.status === "measured"
...
export function meanMeasurement(values: readonly Measurement[], label: string): Measurement {
  if (values.length === 0) return unknownMeasurement(`no ${label} measurements`);
  ...
  for (const value of values) {
    if (value.status !== "measured") continue;
...
function validateOptions(value: unknown): EvaluateOptions {
  ...
  if (
    judge !== undefined &&
    (judge === null || typeof judge !== "object" || typeof (judge as { judge?: unknown }).judge !== "function")
  ) {
  ...
  if (typeof model === "string" && model.length > MAX_COMPLETION_MODEL_CHARACTERS) {
    throw new CoreValidationError(
      "INPUT_TOO_LARGE",
      `Evaluate option 'model' is ${model.length} characters; limit is ${MAX_COMPLETION_MODEL_CHARACTERS}.`,
    );
  }
  const maxOutputCharacters = readOwnField(options, "maxOutputCharacters");
  if (
    maxOutputCharacters !== undefined &&
    (!Number.isSafeInteger(maxOutputCharacters) || (maxOutputCharacters as number) <= 0)
  ) {
    throw new CoreValidationError(
      "INVALID_EVALUATION",
      "Evaluate option 'maxOutputCharacters' must be a positive safe integer.",
    );
  }
  if (
    maxOutputCharacters !== undefined &&
    (maxOutputCharacters as number) > MAX_COMPLETION_OUTPUT_CHARACTERS
  ) {
    throw new CoreValidationError(
      "OUTPUT_TOO_LARGE",
      `Evaluate option 'maxOutputCharacters' is ${maxOutputCharacters}; limit is ${MAX_COMPLETION_OUTPUT_CHARACTERS}.`,
    );
  }
  ...
  if (
    ...
    throw new CoreValidationError(
      "INVALID_EVALUATION",
      `Evaluate option 'timeoutMs' must be an integer between 0 and ${MAX_COMPLETION_TIMEOUT_MS}.`,
    );
  }
  return Object.freeze({
    ...(judge === undefined ? {} : { judge: judge as SemanticJudgeAdapter }),
    ...(model === undefined ? {} : { model: model as string }),
    ...(maxOutputCharacters === undefined ? {} : { maxOutputCharacters: maxOutputCharacters as number }),
    ...(signal === undefined ? {} : { signal: signal as AbortSignal }),
    ...(timeoutMs === undefined ? {} : { timeoutMs: timeoutMs as number }),
  });
}

function validateCompletionAdapter(value: unknown): CompletionAdapter {
  if (value === null || typeof value !== "object" || typeof (value as { complete?: unknown }).complete !== "function") {
    throw new CoreValidationError("INVALID_EVALUATION", "Completion adapter must expose a complete(request, signal?) method.");
  }
  return value as CompletionAdapter;
}

const EVALUATION_INPUT_PREFIX = "\n\n<evaluation_input>\n";
const EVALUATION_INPUT_SUFFIX = "\n</evaluation_input>";

function evaluationPromptLength(candidate: PromptCandidate, evaluationCase: EvaluationCase): number {
  const input = readOwnField(evaluationCase, "input") as string | undefined;
  if (input === undefined) return candidate.prompt.text.length;
...
function preflightEvaluationPrompt(candidate: PromptCandidate, evaluationCase: EvaluationCase): void {
  const length = evaluationPromptLength(candidate, evaluationCase);
  if (length > MAX_EVALUATION_PROMPT_CHARACTERS) {
    throw new CoreValidationError(
      "INPUT_TOO_LARGE",
      `Evaluation prompt for candidate '${candidate.id}' and case '${evaluationCase.id}' is ${length} characters; limit is ${MAX_EVALUATION_PROMPT_CHARACTERS}.`,
    );
  }
}

/** Builds the prompt sent to the completion adapter. Case input is escaped and delimited as data. */
export function buildEvaluationPrompt(candidate: PromptCandidate, evaluationCase: EvaluationCase): string {
  const input = readOwnField(evaluationCase, "input") as string | undefined;
...
function buildCompletionRequest(
  ...
  const model = readOwnField(options, "model") as string | undefined;
  const maxOutputCharacters = readOwnField(options, "maxOutputCharacters") as number | undefined;
  return {
...
export function preflightEvaluationRequests(
  ...
  for (const candidate of candidates) {
    for (const evaluationCase of suite.cases) preflightEvaluationPrompt(candidate, evaluationCase);
  }
...
export async function evaluateCase(
  ...
  const outputLimit =
    (readOwnField(checkedOptions, "maxOutputCharacters") as number | undefined) ?? MAX_COMPLETION_OUTPUT_CHARACTERS;
  const timeoutMs = readOwnField(checkedOptions, "timeoutMs") as number | undefined;
  const signal = readOwnField(checkedOptions, "signal") as AbortSignal | undefined;
  if (timeoutMs === undefined) throwIfCompletionAborted(signal);
  ...
  const text = readOwnField(result, "text") as string;
  const model = (readOwnField(result, "model") as string | undefined) ?? null;
  const finishReason = (readOwnField(result, "finishReason") as string | undefined) ?? null;
  const deterministic = evaluateOutput(text, checkedCase.criteria);
  const measurements = measureCompletion(result);

  const judge = readOwnField(checkedOptions, "judge") as SemanticJudgeAdapter | undefined;
  const rubric = readOwnField(checkedCase, "judgeRubric") as string | undefined;
  let judgeOutcome: JudgeOutcome;
  if (judge === undefined) {
    ...
    const request = buildJudgeRequest({
      ...
      rubric,
      input: (readOwnField(checkedCase, "input") as string | undefined) ?? null,
      expectedOutput: (readOwnField(checkedCase, "expectedOutput") as string | undefined) ?? null,
...
export function aggregateCases(cases: readonly CaseEvaluation[]): CandidateAggregate {
  const deterministicScores = cases.map((entry) =>
    entry.deterministic.score === null
      ? unknownMeasurement(`case '${entry.caseId}' has no weighted criteria`)
      : measured(entry.deterministic.score),
  );
  const judgeScores = cases.map((entry) =>
    entry.judge.status === "judged"
...
export async function evaluateSuite(
  ...
  if (checkedCandidates.length > MAX_EVALUATION_CANDIDATES) {
    throw new CoreValidationError(
      "BUDGET_EXCEEDED",
      `${checkedCandidates.length} candidates exceed the limit of ${MAX_EVALUATION_CANDIDATES}.`,
    );
  }
  const ids = new Set<string>();
  for (const candidate of checkedCandidates) {
    if (ids.has(candidate.id)) throw new CoreValidationError("INVALID_CANDIDATE", `Prompt candidate id '${candidate.id}' is duplicated.`);
  ...
  if (completions > MAX_EVALUATION_COMPLETIONS) {
    throw new CoreValidationError(
      "BUDGET_EXCEEDED",
      `${checkedCandidates.length} candidates x ${checkedSuite.cases.length} cases = ${completions} completions; limit is ${MAX_EVALUATION_COMPLETIONS}.`,
    );
  }
